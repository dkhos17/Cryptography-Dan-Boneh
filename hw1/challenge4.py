...
def findKey(hex):
    return max([total_score(key, hex) for key in range(256)])     
        
def findDecrypt(hexs):
    return max([findKey(hex) for hex in hexs])[1]
    
hexs = [input() for _ in range(int(input()))]
print(findDecrypt(hexs))